<template>
    <h1 class="font-lato mb-20 border-b-2 text-center text-2xl">Modifier le Logiciel Design</h1>
  
    <div class="relative mt-12 mb-20 flex flex-col gap-20 px-5">
      <form @submit.prevent="UpdateLogicielDesign">
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div class="grid grid-cols-1 gap-14">
            <div class="flex h-10 overflow-hidden rounded-2xl text-white">
              ...
              <input class="font-museomoderno w-full border-2 bg-blue-700 p-5" type="text" placeholder="Nom du Logiciel" v-model="LogicielDesign.nom" required >
            </div>
            ...
            <div class="flex h-12 overflow-hidden rounded-2xl text-white">
              ...
              <input
                ...
                v-model="LogicielDesign.description"
                required>
            </div>
  ...
  export default {
    ...
    mounted() {
      ...
      this.getLogicielDesign(this.$route.params.id);
    ...
    methods: {
      previewImage: function (event) {
        ...
        this.LogicielDesign.photo = this.file.name;
      ...
      async getLogicielDesign(id) {
        // Obtenir Firestore
        const firestore = getFirestore();
  
        const docRef = doc(firestore, "LogicielDesign", id);
  
        this.refLogicielDesign = await getDoc(docRef);
  
        if (this.refLogicielDesign.exists()) {
          // Si oui on récupère ses données
          this.LogicielDesign = this.refLogicielDesign.data();
          // Mémorisation photoActuelle
          this.photoActuelle = this.LogicielDesign.photo;
        } else {
        ...
        const spaceRef = ref(storage, "ApercuLogicielDesign/" + this.LogicielDesign.photo);
      ...
      async UpdateLogicielDesign() {
        ...
        if (this.imgModifiee) {
          ...
          let docRef = ref(storage, "ApercuLogicielDesign/" + this.photoActuelle);
          // Suppression photo actuelle
          deleteObject(docRef);
          // création nouvelle photo
          // Référence de l'image à uploader
          docRef = ref(storage, "ApercuLogicielDesign/" + this.LogicielDesign.photo);
          await uploadString(docRef, this.imageData, "data_url").then((snapshot) => {
            console.log("Uploaded a base64 string", this.LogicielDesign.photo);
          });
        }
  
        const firestore = getFirestore();
  
        await updateDoc(doc(firestore, "LogicielDesign", this.$route.params.id), this.LogicielDesign);
