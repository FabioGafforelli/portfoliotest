<template>
    <h1 class="font-rajdhani mb-10 mt-20 border-b-2 text-center text-2xl">Suppression d'un Logiciel Dev</h1>
  
    <div class="relative mt-2 flex flex-col gap-20 px-5">
      <div class="">
        <p class="font-rajdhani w-full bg-blue-700 py-3 text-center text-white">
          Êtes-vous sur de vouloir supprimer ce logiciel Dev ?
        </p>
      </div>
      <form @submit.prevent="DeleteLogicielDev">
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div class="grid grid-cols-1 gap-14">
            <div class="flex h-10 overflow-hidden rounded-2xl text-black">
              ...
              <input class="font-rajdhani w-full border-2 bg-white p-5" placeholder="Nom du Logiciel" v-model="LogicielDev.nom" disabled >
            </div>
            ...
            <div class="flex h-12 overflow-hidden rounded-2xl text-black">
              ...
              <input class="font-rajdhani w-full border-2 bg-white p-5" placeholder="Description du Logiciel" v-model="LogicielDev.description" disabled />
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      async DeleteLogicielDev() {
        const firestore = getFirestore();
  
        await deleteDoc(doc(firestore, "LogicielDev", this.$route.params.id));
  
        // Suppresson de l'image
        const storage = getStorage();
        // Référence du fichier de la photo
        let docRef = ref(storage, "ApercuLogicielDev/" + this.LogicielDev.photo);
