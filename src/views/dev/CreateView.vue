<template>
    <h1 class="font-museomoderno mb-20 mt-10 border-b-2 text-center text-2xl">Ajout d'une compétence en Dev</h1>
    ...
    <div class="relative mt-12 mb-20 flex flex-col gap-20 px-5">
      ...
      <form enctype="multipart/form-data" @submit.prevent="CreateLogicielDev">
        <div class="grid grid-cols-1 gap-4 lg:flex lg:w-[100%] lg:justify-center">
          ...
          <div class="grid grid-cols-1 gap-14">
            <div class="flex h-10 overflow-hidden rounded-2xl text-black">
              ...
              <input class="font-museomoderno w-full border-2 bg-white p-5" type="text" placeholder="Nom du Logiciel" v-model="LogicielDev.nom" required />
            </div>
  ...
  export default {
    ...
    methods: {
      async getLogicielDev() {
        ...
        const q = query(dbLogicielDev, orderBy("nom", "asc"));
        // Récupération de la liste des pays à partir de la query
        // La liste est synchronisée
        await onSnapshot(q, (snapshot) => {
          this.ListeLogicielDev = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
          console.log("Liste des logiciels dev", this.ListeLogicielDev);
        });
      },
